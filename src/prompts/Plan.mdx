...
```
git add {props.planFilePath}
git commit -m "{props.commitPrefix || '📝'} docs({props.ticketCategory}): create plan for {props.ticketId}"
git pull --rebase origin {props.mainBranch || 'main'} && git push
```
